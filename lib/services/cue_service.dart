...
class CueService {
  ...
  FlutterTts? _tts;
  AudioPlayer? _cachePlayer;
  bool _engineInitAttempted = false;
  double _volume = 1.0;
  double _speechRate = 0.52;
  final Map<String, String> _promptCache = <String, String>{};
  Future<void>? _cacheInitFuture;
  String? _cacheDirectoryPath;

  bool get supportsVoiceCues => _supportsVoiceCues;
  bool get geminiVoiceOverConfigured => GeminiConfig.isConfigured;
  ...
  Future<void> announceExercise(
    ...
    await _speakPrompt('Next exercise: $text.', shouldSpeak: shouldSpeak);
  }

  Future<void> announceRest({bool shouldSpeak = true}) async {
    await _speakPrompt('Next rest. Take a break.', shouldSpeak: shouldSpeak);
  }
  ...
  Future<void> speakCount(int seconds, {bool shouldSpeak = true}) async {
    if (!shouldSpeak || seconds < 1) {
      return;
    }

    await _speakPrompt('$seconds', shouldSpeak: shouldSpeak);
  }
  ...
  Future<void> announceCompletion() async {
    await _speakPrompt('Workout complete. Great job.', shouldSpeak: true);
  }
  ...
  Future<void> _speakPrompt(String text, {required bool shouldSpeak}) async {
    final trimmed = text.trim();
    await _ensureEnginesReady();
    if (!shouldSpeak || trimmed.isEmpty || _tts == null) {
      return;
    }
    ...
    try {
      ...
      final cacheKey = _buildPromptKey(trimmed);
      final cachedPath = _promptCache[cacheKey];
      if (cachedPath != null && await File(cachedPath).exists()) {
        await _playCachedFile(cachedPath);
        return;
      }

      final generatedPath = await _synthesizePromptToFile(trimmed, cacheKey);
      if (generatedPath != null) {
        _promptCache[cacheKey] = generatedPath;
        await _persistPromptCache();
        await _playCachedFile(generatedPath);
        return;
      }
    } catch (_) {
      // Fallback to direct TTS when cache generation or playback is unavailable.
    }
  ...
  Future<void> _ensureCacheReady() async {
    ...
    final pending = _cacheInitFuture;
    if (pending != null) {
      await pending;
      return;
    }

    final initFuture = _initializeCache();
    _cacheInitFuture = initFuture;
    await initFuture;
  }

  Future<void> _ensureEnginesReady() async {
    if (!_supportsVoiceCues) {
      return;
    }

    if (_engineInitAttempted) {
      return;
    }
  ...
  Future<void> _initializeCache() async {
    ...
    final cacheDirectory = Directory(
      '${directory.path}${Platform.pathSeparator}voice_prompt_cache',
    );
    ...
    final indexFile = File(
      '${cacheDirectory.path}${Platform.pathSeparator}prompt_index.json',
    );
    ...
    try {
      ...
      for (final entry in decoded.entries) {
        ...
        if (await File(value).exists()) {
          _promptCache[key] = value;
        }
      }
    } catch (_) {
      // Corrupt cache index should not break voice cues.
    }
  }

  Future<void> _persistPromptCache() async {
    final cachePath = _cacheDirectoryPath;
    if (cachePath == null) {
      return;
    }

    final indexFile = File(
      '$cachePath${Platform.pathSeparator}prompt_index.json',
    );
    await indexFile.writeAsString(jsonEncode(_promptCache));
  }

  Future<String?> _synthesizePromptToFile(String prompt, String cacheKey) async {
    final cachePath = _cacheDirectoryPath;
    final tts = _tts;
    if (cachePath == null || tts == null) {
      return null;
    }

    final extension = defaultTargetPlatform == TargetPlatform.iOS ? 'caf' : 'wav';
    final outputPath = '$cachePath${Platform.pathSeparator}$cacheKey.$extension';

    try {
      await tts.synthesizeToFile(prompt, outputPath, true);
  ...
  Future<void> _playCachedFile(String filePath) async {
    final player = _cachePlayer;
    if (player == null) {
      return;
    }
  ...
  String _buildPromptKey(String text) {
    var hash = 2166136261;
  ...
  Future<void> stop() async {
    await _tts?.stop();
    await _cachePlayer?.stop();
  }

  Future<void> dispose() async {
    await _tts?.stop();
    await _cachePlayer?.dispose();
  }
}
